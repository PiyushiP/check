...
def display_one_thermal_image(one_image, its_label):
  plt.imshow(one_image)
  print ("Label = " + its_label)
  print ("image shape: ",one_image.size)
...
def get_cars_classes_dict(testY,testLabels):
  ...
  for i,label in enumerate(testY):
      car_dict[np.argmax(label)]=testLabels[i]
      #print(len(car_dict))
      length=len(car_dict)
      if (length==number_of_classes):
          break
  #print(sorted(car_dict.items()))
  #print(car_dict)
  return car_dict,number_of_classes

def plot_test_image(testX, image_index, predictions_array, true_binary_labels, car_dict=None):
    """
    ...
    if (car_dict==None):
      plt.xlabel("{} predicted: {} {:2.0f}% (true: {})".format(image_index, predicted_binary_label,
                                  100*np.max(single_predictions_array),
                                  true_binary_label),
                                  color=color)
    else:
      ##print(car_dict[predicted_binary_label])
       plt.xlabel("{} predicted: {} {:2.0f}% (true: {})".format(image_index, car_dict[predicted_binary_label],
                                  100*np.max(single_predictions_array),
                                  car_dict[true_binary_label]),
                                  color=color)
...
def plot_car_sample_predictions(testX, predictions_array, testY,number_of_classes=3,num_rows = 10, num_cols = 4, width=None, height=None, is_random=True):
    """
        this method plots a sample of predictions from the test dataset and highlight correct and wrong predictions
        testX: this is the test dataset
        image_index: index of the image that we will plot from the test dataset
...
def plot_car_sample_predictions_v2(testX, testY, testLabels, predictions_array, num_rows = 10, num_cols = 4, width=None, height=None, is_random=True, plot_bars=True):
    """
        this method plots a sample of predictions from the test dataset and highlight correct and wrong predictions
        testX: this is the test dataset
        image_index: index of the image that we will plot from the test dataset
    ...
    #number_of_classes=np.unique(testLabels)
    ...
    for i in range(num_images):
        ...
        #if (plot_bars==True):
        plt.subplot(num_rows, 2*num_cols, 2*i+1)
        plot_test_image(testX, image_index, predictions_array[image_index], true_binary_labels, car_dict)
    #---------------
        plt.subplot(num_rows, 2*num_cols, 2*i+2)
        plot_value_array(image_index, predictions_array[image_index], true_binary_labels, number_of_classes)
...
def plot_misclassifications(testX, testY, testLabels, predictions_array, num_rows = 10, num_cols = 4, width=None, height=None, is_random=True, plot_bars=True):
    """
        this method plots a sample of predictions from the test dataset and highlight correct and wrong predictions
        testX: this is the test dataset
        image_index: index of the image that we will plot from the test dataset
    ...
    for i in range(num_images):
        ...
        #print(predicted_binary_label,'   ', true_binary_labels[image_index],'   ',predictions_array[image_index].shape)
        ...
        #---------------
            ...
            plot_value_array(image_index, predictions_array[image_index], true_binary_labels, number_of_classes)
            count=count+1
            misclassified_indices[image_index]=[car_dict[true_binary_labels[image_index]],car_dict[predicted_binary_label]]
        
        #else:
       #    plt.subplot(num_rows, num_cols, i)
       #     plot_test_image(testX, image_index, predictions_array[image_index], true_binary_labels, car_dict) 
    print('number of misclassications: ', count)
    print('number of images:', nb_images)
    print('rate of misclassications: ', (100*count/num_images),'%')
...
def misclassification_stats(misclassification_dict):
  ...
  for key in misclassification_dict:
      ...
      if (len(true_label)==5):
        gen_of_true_label =true_label[3] +' '+true_label[4]
      else:
        gen_of_true_label =true_label[3] 

      if (len(predicted_label)==5):
        gen_of_predicted_label =predicted_label[3]+' '+predicted_label[4]
      else:
        gen_of_predicted_label =predicted_label[3]
      ...
      if(brand_of_true_label ==brand_of_predicted_label):
        if(model_of_true_label==model_of_predicted_label):
          if(gen_of_true_label!=gen_of_predicted_label):
            #print(misclassification_dict[key][0], ', ',misclassification_dict[key][1])
            gen=gen+1 
        else: 
            #print(misclassification_dict[key][0], ', ',misclassification_dict[key][1])
            model = model+1
  ...
  return gen,model,brand, (gen/dict_size), (model/dict_size), (brand/dict_size), dict_size
